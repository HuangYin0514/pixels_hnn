...
class DAE_NN(nn.Module):
    ...
    def criterion(self, t, coords, labels):
        # d_coords = self(t, coords)

        # q, qt, lambdas = torch.tensor_split(d_coords, (self.config.dof, self.config.dof * 2), dim=-1)
        # d_coords = torch.cat([q, qt], dim=-1)

        # noise = torch.randn(1, device=self.config.device, dtype=self.config.dtype)
        # d_coords = d_coords + noise

        rk4_solver = RK4()
        pred = rk4_solver.step(self, t0=0, y0=coords, dt=0.01)
